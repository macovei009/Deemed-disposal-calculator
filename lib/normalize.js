...
export function normalizeRows(rows) {
  ...
  const normalized = rows
    .map((raw, idx) => {
      ...
      const isinKey = findKey(raw, ['isin', 'instrument']);
      ...
      const isBuy = actionLower.includes('market buy') || actionLower === 'buy' || actionLower === 'b' || actionLower.includes('limit buy');
      const isSell = actionLower.includes('market sell') || actionLower === 'sell' || actionLower === 's' || actionLower.includes('limit sell');
