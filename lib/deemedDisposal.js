...
export async function computeDeemedDisposal({ transactions, taxYear }) {
  ...
  for (const lot of remaining) {
    ...
    // CRITICAL: Only ignore if classification says NOT ETF (isEtf === false)
    if (classification.isEtf === false) {
      ignored.push({ isin: lot.isin, brokerTicker: lot.ticker, buyDate: lot.dateISO, reason: 'Not an ETF (authoritative classification)' });
      continue;
    }
    ...
    if (classification.isEtf === null) {
      notes.push(`ETF classification unknown (confidence: ${classification.confidence}); treating as ETF.`);
    }
